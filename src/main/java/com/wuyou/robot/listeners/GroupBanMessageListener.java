...
public class GroupBanMessageListener {
		...
		for (String banMessage : list) {
			if (message.contains(banMessage)) {
				if (PowerUtils.powerCompare(msg, fromQQ, sender)) {
					...
					sender.SENDER.sendGroupMsg(fromGroup, CQ.at(fromQQ) + "发送关键词[" + banMessage + "]， 抽中禁言" + a / 2
	...
	private void luck(GroupMsg msg, MsgSender sender) {
		...
		if (a <= 90) {
			if (PowerUtils.powerCompare(msg, fromQQ, sender)) {
				sender.SENDER.sendGroupMsg(fromGroup, CQ.at(fromQQ) + "抽中禁言" + min + "分");
			...
			sender.SENDER.sendGroupMsg(fromGroup, CQ.at(fromQQ) + "恭喜轮空");
		}
	}
