...
public class RequestListener {
	...
	public void groupAddRequest(GroupAddRequest request, MsgSender sender) {
		...
		if (list.contains(qq)) {
			...
			sender.SENDER.sendGroupMsg(fromGroup, "已经拒绝黑名单成员[" + request.getQQ() + "]("
...
//		String qq = increase.
		...
		try {
			...
			if (statService.getStat(fromGroup) == 1) {
				...
				sender.SENDER.sendGroupMsg(fromGroup, util.getCQCode_At(beingOperateQQ).toString() + welcomeMessage);
			}
