...
public class GroupManagementListener {
	...
	public void kickMember(GroupMsg msg, MsgSender sender) {
		...
		if (set.size() == 0) {
			sender.SENDER.sendGroupMsg(fromGroup, "指令不合法,请至少艾特一位群成员");
			return;
		}
		if (getPower(msg, sender)) {
			...
			sender.SENDER.sendGroupMsg(fromGroup, CQ.at(fromQQ) + str);
		}
	}
	...
	public void banMember(GroupMsg msg, MsgSender sender) {
		...
		if (getPower(msg, sender)) {
			...
			try {
				...
				sender.SENDER.sendGroupMsg(fromGroup, "指令不合法,请在后面添加纯数字,单位:分钟");
				return;
			}
			try {
				StringBuilder str = new StringBuilder("\n禁言:");
				...
				sender.SENDER.sendGroupMsg(fromGroup, CQ.at(fromQQ) + "指令不合法");
			}
		}
	}
	...
	public void allBlackMember(GroupMsg msg, MsgSender sender) {
		...
		if (num == 0) {
			sender.SENDER.sendGroupMsg(fromGroup, "暂无黑名单记录");
		} else {
			sender.SENDER.sendGroupMsg(fromGroup, CQ.at(fromQQ) + str);
		}
	}
	...
	public void changeNick(GroupMsg msg, MsgSender sender) {
		...
		if (PowerUtils.getPowerType(fromGroup, fromQQ, sender) > 1) {
			...
			if (message.startsWith("改名") && set.size() == 1 && CQ.getAt(message).equals(thisQQ)) {
				...
				if (CQ.getAt(name) == null) {
					...
					sender.SENDER.sendGroupMsg(fromGroup, "已把我的名字改为: \"" + name + "\"");
				} else {
					sender.SETTER.setGroupCard(fromGroup, thisQQ, "");
					sender.SENDER.sendGroupMsg(fromGroup, "已取消我的群名片");
				}
				return;
			}
		}
		if (getPower(msg, sender)) {
			try {
				...
				sender.SENDER.sendGroupMsg(fromGroup, CQ.at(fromQQ) + "\n指令不合法");
			}
		}
	}
	...
	public void banList(GroupMsg msg, MsgSender sender) throws IOException {
		...
		if (list == null) {
			sender.SENDER.sendGroupMsg(fromGroup, CQ.at(fromQQ) + "当前群内没有被禁言的群员");
			return;
		}
		int num = 0;
	...
	private boolean getPower(GroupMsg msg, MsgSender sender) {
		...
		if (PowerUtils.getPowerType(group, msg.getQQ(), sender) > 1) {
			...
			if (power == 3 || power == 1) {
				...
				sender.SENDER.sendGroupMsg(group, "添加失败,我没有管理员权限!");
		} else
			sender.SENDER.sendGroupMsg(group, "操作失败,你不是我的管理员!");
		return false;
	}
