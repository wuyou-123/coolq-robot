...
public class GroupOtherListeners {
	...
	public void sendLike(GroupMsg msg, MsgSender sender) {
		...
		try {
			if (sender.SENDER.sendLike(fromQQ, 10)) {
				sender.SENDER.sendGroupMsg(fromGroup, CQ.at(fromQQ) + "已点赞");
			} else {
				sender.SENDER.sendGroupMsg(fromGroup, CQ.at(fromQQ) + "点赞失败");
			}
		} catch (Exception e) {
			sender.SENDER.sendGroupMsg(fromGroup, CQ.at(fromQQ) + "点赞失败");
		}
	...
	public void sendMenu(GroupMsg msg, MsgSender sender) {
		sender.SENDER.sendGroupMsg(msg.getGroup(), "http://wuyourj.club/menu.html");
	}
	...
	public void autistic1(GroupMsg msg, MsgSender sender) {
		...
		if (PowerUtils.powerCompare(msg, fromQQ, sender)) {
			...
			sender.SENDER.sendGroupMsg(fromGroup, CQ.at(fromQQ) + "自闭成功" + CQCodeUtil.build().getCQCode_Face("178"));
		} else {
